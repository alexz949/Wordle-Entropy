(async () => {
  ...
  // ---------------- Shadow root helper ----------------
  // Chrome extensions can access open OR closed shadow roots via chrome.dom.openOrClosedShadowRoot(). :contentReference[oaicite:7]{index=7}
  ...
  // ---------------- DOM readers (shadow + non-shadow fallback) ----------------
  ...
  // A) Shadow DOM version: game-row/game-tile with attributes letter/evaluation
  ...
  // B) Non-shadow DOM version: tiles are divs with data-state and text letters
  function readDivBoard() {
    // These selectors mirror what people use in automation after NYT removed Shadow DOM:
